...
void setup()
{
  ...
  Tensao_ref_fonte   = Maximo_ICR1 * 0.40;
  ...
  TCNT2  = 193;             //inicia Timer2 para contar a partir de 193 e estabelecer uma frequencia de 8kHz
...
void Algoritmo_MPPT(float corrente, float tensao) 
{
  ...
  OCR1A = Tensao_ref_k - Tempo_morto; //atualização do dutty do PWM do pino PB1 (pino D09 no Arduino Nano)
  OCR1B = Tensao_ref_k + Tempo_morto; //atualização do dutty do PWM do pino PB2 (pino D10 no Arduino Nano)
